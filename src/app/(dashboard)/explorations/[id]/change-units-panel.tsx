...
export function ChangeUnitsPanel({ requirementId }: Props) {
  ...
  const loadItems = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error('加载 Change Queue 失败')
      ...
      setError(err instanceof Error ? err.message : '加载 Change Queue 失败')
    } finally {
      setLoading(false)
    }
  }, [requirementId])
  ...
  return (
    <section className="app-panel p-4 sm:p-5">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <h3 className="text-sm font-semibold uppercase tracking-[0.22em] text-slate-600">Change Queue</h3>
          <p className="mt-2 text-sm leading-6 text-slate-500">
            把问题收敛成正式变更对象，明确原因、影响范围和是否需要重新评审。
          </p>
        </div>
      ...
      <div className="mt-5 grid gap-3 lg:grid-cols-2">
        <Input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Change 标题" />
        <select
          value={riskLevel}
      ...
      <div className="mt-4 flex items-center gap-3">
        <Button onClick={handleCreate} disabled={creating}>
          {creating ? '创建中...' : '新增 Change'}
        </Button>
        {createError ? <p className="text-sm text-red-600">{createError}</p> : null}
      </div>

      {loading ? <p className="mt-5 text-sm text-slate-400">加载 Change Queue...</p> : null}
      ...
      {!loading && !error && items.length === 0 ? (
        <div className="mt-5 rounded-[24px] border border-dashed border-slate-300/80 bg-slate-50/80 p-5 text-sm text-slate-500">
          还没有正式变更对象。先在 Issue Queue 中定位问题，再把需要正式推进的项收敛到 Change Queue。
        </div>
      ) : null}
              ...
              {draft?.open ? (
                <div className="mt-4 space-y-3 rounded-[20px] border border-slate-200/80 bg-slate-50/70 p-4">
                  <div className="grid gap-3 lg:grid-cols-2">
                    <Input value={draft.title} onChange={(event) => updateEditDraft(item.id, { title: event.target.value })} placeholder="Change 标题" />
                    <select
